  ...
  <BaseModal title="Profile" @close="$emit('close')" width="max-w-[470px]" height="h-auto">
    ...
    <ul class="flex w-full flex-col">
      <li class="mb-3 flex items-center">
        <inline-svg class="mr-2" src="user-profile/email.svg" />
        <a :href="`mailto:${user.email}`"> {{ user.email }} </a>
      </li>
      <li class="mb-3 flex items-center">
        <inline-svg class="mr-2" src="user-profile/github.svg" />
        <a :href="user.github" target="_blank"> {{ 'github' }} </a>
      </li>
      <li class="mb-3 flex items-center">
        <inline-svg class="mr-2" src="user-profile/linkedin.svg" />
        <a :href="user.linkedin" target="_blank"> {{ 'Linkedin' }} </a>
      </li>
      <li class="mb-3 flex items-center">
        <inline-svg class="mr-2" src="user-profile/phone.svg" />
        <a :href="`tel:${user.phone}`"> {{ user.phone || 'Phone number' }} </a>
      </li>
      <li class="mb-3 flex items-center">
        <inline-svg class="mr-2" src="user-profile/telegram.svg" />
    ...
    >
      <inline-svg class="mr-2" src="user-profile/file-pdf.svg" />
